## YYYY-MM-DD - [Optimize len(instances.list_all) O(N) to O(K) in server.py]
**Learning:** Checking the number of instances an owner has by calling `len(instances.list_all(for_owner_id=...))` creates an O(N log N) bottleneck due to sorting overhead inside `list_all`, even though the result only needs the count.
**Action:** Introduced `count_for_owner` in `InstanceManager` which provides an O(K) lookup of the user's instance count using a generator expression and the `_owner_to_ids` dictionary, replacing expensive `list_all` materialization and length evaluation.
