...
## 2025-05-18 - [Optimize InstanceManager instance counting for specific owners]
**Learning:** Checking the number of instances an owner has by calling `len(instances.list_all(for_owner_id=X))` performs an unnecessary O(N) list construction and iteration across instances, which adds memory and CPU overhead especially on hot paths like queue checking in `_enqueue_message`.
**Action:** Implemented a direct O(1) dictionary lookup via a new `count_for_owner(owner_id)` method that returns `len(self._owner_to_ids.get(owner_id, set()))`, bypassing the list materialization entirely and replacing usages in `server.py`.
