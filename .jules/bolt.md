...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
## 2024-05-18 - [Optimize len(list_all()) to O(1) dictionary count]
**Learning:** Calling `len(instances.list_all(for_owner_id=X))` creates an O(N log N) overhead due to sorting and object instantiation when simply checking the count of user instances.
**Action:** Added `count_for_owner` to `InstanceManager` which checks the length of the internal `_owner_to_ids` dictionary directly in O(1) time.
