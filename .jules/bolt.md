...
## YYYY-MM-DD - [Optimize len(list_all()) to count_for_owner()]
**Learning:** `InstanceManager` suffered from O(N log N) overhead when counting instances for a specific owner by materializing and sorting a list via `len(instances.list_all(for_owner_id=X))`.
**Action:** Replaced `len(instances.list_all(...))` with `instances.count_for_owner(...)` utilizing a generator expression and lookup in O(K) time without sorting or list allocation.
