...
## 2024-05-20 - [Optimize broad collection iterations in instance_manager]
**Learning:** `InstanceManager.list_all()` performs a full sort (`sorted(instances, key=lambda i: i.id)`) on all instances before returning them. This sorting is O(N log N). However, operations like checking if any instance is processing, calculating total queue size, clearing all queues on shutdown, or scanning for an active agent (in `agent_manager.py` and `server.py`) do not require ordered output, making the sorting overhead completely redundant.
**Action:** Introduced an `iter_all()` method on `InstanceManager` that returns an unsorted snapshot (`list(self._instances.values())`). Updated broad iterations across the system to use `iter_all()` instead of `list_all()` to avoid unnecessary sorting. Explicitly returning a list snapshot instead of a generator is critical to avoid `RuntimeError: dictionary changed size during iteration` when callers `await` tasks inside the iteration loop.
