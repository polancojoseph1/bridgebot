...
## 2024-05-18 - [Optimize InstanceManager instance counting O(M) to O(1)]
**Learning:** `InstanceManager.list_all(for_owner_id=X)` was frequently called inside `server.py` just to calculate the length (count of instances for a specific owner) for display purposes or to check if an owner had multiple instances. Since `list_all` sorts the filtered instances `sorted(instances, key=lambda i: i.id)`, this incurred unnecessary O(M log M) sorting overhead just to get a length.
**Action:** Introduced a new `count_for_owner(owner_id)` method on `InstanceManager` that returns `len(self._owner_to_ids.get(owner_id, set()))`. This provides an O(1) lookup of the instance count without constructing a new list or sorting it. Updated `server.py` and `instance_manager.py` to use this new fast-path method wherever only the count is needed.
