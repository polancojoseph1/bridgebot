...
## 2025-05-01 - [Optimize O(N log N) sorting overhead in InstanceManager]
**Learning:** Checking properties across all instances via `instances.list_all()` introduces an unnecessary O(N log N) sorting bottleneck because `list_all()` returns a sorted list.
**Action:** Used a new `iter_all()` method returning an unsorted list snapshot (`list(self._instances.values())`) to change these O(N log N) bottlenecks into O(N) linear scans when order does not matter.
