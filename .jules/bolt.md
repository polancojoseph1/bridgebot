...
## YYYY-MM-DD - Replace list_all instance counting
**Learning:** Calling len(list_all) to count instances incurs unnecessary O(N) allocation and O(N log N) sorting overhead when checking instance count.
**Action:** Replaced len(list_all) checks with an O(1) count_for_owner generator expression.
