...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
## 2025-05-18 - [Optimize instance count for owner]
**Learning:** `len(instances.list_all(for_owner_id=owner_id))` was called frequently to determine the number of instances an owner has. This creates an O(N log N) overhead due to sorting the filtered list.
**Action:** Replaced these calls with a new O(1) method `InstanceManager.count_for_owner(owner_id)` which directly checks the length of the set in `_owner_to_ids`.
