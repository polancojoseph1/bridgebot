...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
>>>>>>> main
## YYYY-MM-DD - [Optimize len(list_all()) to O(1) dictionary lookup]
**Learning:** Using `len(instances.list_all(for_owner_id=...))` computes the length of a newly allocated, sorted list of instances on each call. This overhead is O(N log N) within `list_all()`, which slows down frequent queue size checks in `server.py`.
**Action:** Replaced `len(instances.list_all(...))` with a new `instances.count_for_owner(...)` method that directly checks `len(self._owner_to_ids.get(owner_id, set()))`, dropping the check's complexity to O(1).
