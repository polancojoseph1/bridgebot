...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
>>>>>>> main
## 2024-10-24 - Optimize instance counting
**Learning:** Computing the length of instances.list_all() introduces unnecessary O(N log N) sorting overhead when only a count is needed.
**Action:** Replaced len(instances.list_all()) with an O(1) instances.count_for_owner() method.
