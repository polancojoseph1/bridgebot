...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
>>>>>>> main
## YYYY-MM-DD - [Optimize instance_manager.list_all list generation and sorting to O(1)]
**Learning:** `InstanceManager.list_all()` was frequently called to check counts by doing list generation and possibly sorting instances just to get a count. This generates unnecessary objects and computation overhead.
**Action:** Use an O(1) dictionary lookup by returning the length of the `_owner_to_ids` set directly when we only need the count.
