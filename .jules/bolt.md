...
## 2025-05-15 - [Optimize InstanceManager count for owner]
**Learning:** `len(instances.list_all(for_owner_id=owner_id))` computes the length of the sorted list returned by `list_all`, which incurs O(N log N) sorting overhead.
**Action:** Implemented `count_for_owner` using an O(1) dictionary lookup (`len(self._owner_to_ids.get(owner_id, set()))`) and replaced the expensive calls.
