...
## 2026-05-31 - [Optimize InstanceManager instance iteration overhead]
**Learning:** The `list_all()` method in `InstanceManager` was performing an O(N log N) sort using `sorted()` on every invocation. When this was called simply to check properties across all instances—such as in `_is_any_processing()` or `_total_queue_size()`—the sorting overhead was entirely redundant and became a scaling bottleneck inside high-frequency processing loops.
**Action:** Introduced an `iter_all()` method to `InstanceManager` that bypasses sorting and directly returns an unsorted `list(self._instances.values())`. Switched high-frequency aggregation loops to use `iter_all()` to eliminate O(N log N) overhead, downgrading these checks to pure O(N).
