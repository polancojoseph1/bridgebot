...
## 2025-05-18 - [Optimize len(instances.list_all()) O(N log N) to O(1)]
**Learning:** Checking the number of instances a user has by calling `len(instances.list_all(for_owner_id=owner_id))` causes an unnecessary O(N log N) sorting overhead because `list_all` always sorts the returned list.
**Action:** Introduce and use an O(1) `count_for_owner(owner_id)` method that simply returns the length of the `_owner_to_ids` set for that owner, bypassing the sorting step entirely for count checks.
