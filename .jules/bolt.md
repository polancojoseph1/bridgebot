...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
>>>>>>> main
## 2024-03-24 - [Optimize instance_manager.list_all O(n) to O(1)]
**Learning:** `InstanceManager.list_all()` was frequently called just to count instances for a specific owner, resulting in unnecessary O(N) list generation and iteration over all instances across all users.
**Action:** Replaced `len(instances.list_all(for_owner_id=owner_id))` with a new `count_for_owner()` method that leverages the existing `_owner_to_ids` dictionary index for an O(1) lookup.
