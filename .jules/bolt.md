...
## 2025-05-01 - [Optimize len(list_all()) to O(1) dictionary lookup]
**Learning:** `len(instances.list_all(for_owner_id=...))` generates a complete, sorted list of a user's instances merely to count them, incurring O(M log M) overhead. Calling this repeatedly in server logic creates an unnecessary performance bottleneck.
**Action:** Replaced `len(instances.list_all())` with an O(1) dictionary lookup `count_for_owner()` that utilizes the existing `_owner_to_ids` mapping, avoiding the list generation and sorting entirely.
