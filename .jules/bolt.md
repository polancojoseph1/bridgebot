...
**Action:** Pre-compile regular expressions using `re.compile()` at the module level to avoid repeated compilation and cache-lookup overhead during runtime execution.
>>>>>>> main
## 2026-06-01 - [Optimize counting instances for an owner]
**Learning:** `InstanceManager.list_all()` performs filtering and sorting over all instances and returns a list. In several places in `server.py`, the length of this returned list was checked solely to count a user's instances, which unnecessarily instantiates and sorts a list.
**Action:** Implemented `count_for_owner(owner_id)` which performs an O(1) count by retrieving the length of the internal set `_owner_to_ids[owner_id]`. Replaced occurrences of `len(instances.list_all(...))` where only the count was needed with the new optimized method.
