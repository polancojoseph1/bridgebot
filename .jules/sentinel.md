...
## 2026-06-25 - Fix CSRF Vulnerability in API Key Generation
**Vulnerability:** The `/api/generate-bc-key` endpoint in `setup_wizard_ui.py` used a `GET` request method to perform a state-modifying action (generating and storing a random API key in `.env`).
**Learning:** `GET` requests are designed to be safe and idempotent, meaning they should not modify server state. Using `GET` for state-modifying endpoints makes the application vulnerable to Cross-Site Request Forgery (CSRF) through simple mechanisms like `<img>` tags on malicious sites, as well as unintended execution by browser pre-fetching or web crawlers.
**Prevention:** Always use the HTTP `POST` method (or other appropriate state-modifying methods like `PUT`, `PATCH`, `DELETE`) for any endpoints that generate, save, or delete data (e.g., API keys, configuration, user data). Ensure frontend `fetch` calls are updated to include `{method: 'POST'}` accordingly.
