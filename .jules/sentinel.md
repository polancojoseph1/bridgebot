...
## 2024-05-30 - Information Exposure (CWE-209) in Telegram Error Messages
**Vulnerability:** Several exception handlers in `server.py` appended raw exception details (`str(e)` or `{e}`) to Telegram messages sent directly to users (e.g., `send_message(chat_id, f"Error processing message: {e}")`), leaking internal application details, API limitations, or tracebacks to end-users.
**Learning:** Exposing internal system errors through chat interfaces provides malicious actors with deep visibility into application state, backend library failures, or internal paths, which aids reconnaissance for secondary attacks.
**Prevention:** In user-facing endpoints or chat handlers, always log full exception details strictly server-side using `logger.error("Message: %s", e, exc_info=True)`. Only send generic, sanitized messages like "An internal error occurred. Please try again later." to the end user.
