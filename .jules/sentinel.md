...
## $(date +%Y-%m-%d) - SSRF Protection Bypass via Multiple DNS Records (DNS Rebinding)
**Vulnerability:** The pre-flight IP validation check in `_is_safe_url` only inspected the first IP address returned by `socket.getaddrinfo`. An attacker could configure a domain to resolve to multiple IP addresses (e.g., a public IP and a private internal IP), bypassing the security check if the public IP was validated first, while the subsequent HTTP request might use the private IP.
**Learning:** Using `socket.gethostbyname` or validating only a single result from `socket.getaddrinfo` is insufficient for SSRF protection because it ignores secondary IP records which can route traffic to internal networks.
**Prevention:** Always iterate over all IP addresses returned by `socket.getaddrinfo` and ensure that *every* resolved IP address is safe (not private, loopback, link-local, etc.) before allowing the URL.
