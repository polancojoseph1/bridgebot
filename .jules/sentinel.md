...
## 2025-05-24 - [Fix Information Exposure in error messages]
**Vulnerability:** The application was exposing internal error details and raw exceptions (e.g. `Exception` objects and traceback info via string formatting like `{e}`) in messages sent to users via `send_message()`. This could leak sensitive implementation details, stack traces, or internal server state to potential attackers, which is an Information Exposure (CWE-209) vulnerability.
**Learning:** Returning or logging raw exception messages directly to the client can inadvertently expose system internals, database schema details, file paths, or third-party API keys if they are part of the error message.
**Prevention:** Always log the full exception details on the server side using `logger.error("...", exc_info=True)` for debugging purposes, but return a generic, safe error message (e.g., "An error occurred. Please try again.") to the user.
