...
def format_agent_list(instances: InstanceManager) -> str:
    ...
    # Pre-calculate mapping to avoid O(N*M) lookup bottleneck
    running_instances = {
        getattr(inst, "agent_id"): inst
        for inst in instances.list_all()
        if getattr(inst, "agent_id", None) is not None
    }
    # Update cache for consistency
    _agent_instance_map.update({k: v.id for k, v in running_instances.items()})

    lines = [f"<b>Agents ({len(agents)}):</b>"]
    for agent in agents:
        running_inst = running_instances.get(agent.id)
        if running_inst:
