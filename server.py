...
async def _flush_mg_async(mg_id: str) -> None:
    ...
    try:
        ...
        if items:
            try:
                logger.error("Error processing media group: %s", e, exc_info=True)
...
async def _instance_queue_worker(inst: Instance) -> None:
    ...
    while True:
        ...
        try:
            ...
            try:
                logger.error("Error processing your message: %s", e, exc_info=True)
    ...
    if _oneshot_match:
        ...
        async def _oneshot_enqueue():
            try:
                ...
                logger.error("Error sending to @%s: %s", target_ref, e, exc_info=True)
...
async def _handle_document_upload(chat_id: int, file_id: str, dest_path: str, file_name: str) -> None:
    ...
    try:
        ...
        logger.error("Failed to save %s: %s", file_name, e, exc_info=True)
...
async def _process_photo_message(chat_id: int, file_id: str, caption: str = "", instance=None, user_id: int = 0, extra_file_ids: list | None = None) -> None:
    ...
    try:
        ...
        logger.error("Failed to download photo: %s", e, exc_info=True)
...
async def _process_voice_message(chat_id: int, file_id: str, caption: str = "", instance=None, user_id: int = 0) -> None:
    ...
    try:
        ...
        logger.error("Failed to transcribe voice: %s", e, exc_info=True)
...
async def _handle_command(chat_id: int, text: str, user_id: int = 0) -> None:
    ...
    if cmd == "/stop":
        ...
        try:
            ...
            await send_message(chat_id, "Collab module error.")
            return
        ...
        try:
            ...
            await send_message(chat_id, "BridgeNet module error.")
            return
        ...
        try:
            ...
            await send_message(chat_id, f"Failed to connect to {peer_name}.")
            return
