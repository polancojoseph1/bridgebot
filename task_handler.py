"""Task list manager for the tasks file.
...
def _parse_tasks() -> list[dict]:
    """Parse tasks file and return list of {number, status, timestamp, text}.
...
def _write_tasks(tasks: list[dict]) -> None:
    """Rewrite tasks file with renumbered tasks and status markers."""
    lines = [_HEADER]
